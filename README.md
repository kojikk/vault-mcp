...
Транспорт остаётся тупым: конвертацию форматов и понимание контента делает мозг (агент мультимодален), MCP лишь двигает байты. Бинарники в вольт кладутся через ФС, не через MCP.
...
| Поиск/дедуп | `search` (параметр `scope`: `all` / `knowledge` / `raw`) |
| Аудит здоровья | `lint` (сироты, битые ссылки, устаревшие entity, несвязанное сырьё, открытые противоречия) |
| Запись (additive, есть `idempotency_key`) | `create_note`, `append_to_home`, `add_raw` (текст → append-only `_raw/`), `update_memory`, `update_index`, `update_hot` |
